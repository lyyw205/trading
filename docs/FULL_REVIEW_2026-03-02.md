...
> Phase 3~4를 1-2주 내에 완료하는 것을 권장합니다.
  ...
  1. 테스트 커버리지 공백 (가장 큰 우려)

  리뷰 마지막에 언급했지만, 이건 Phase 1~5에 포함되지 않았습니다:

  ┌─────────────────────────────────────┬────────┬─────────────────────────────────┐
  │              핵심 모듈              │ 테스트 │             위험도              │
  ├─────────────────────────────────────┼────────┼─────────────────────────────────┤
  │ strategies/buys/ (매수 전략)        │ 0줄    │ 매수 로직 버그 = 잘못된 시점에  │
  │                                     │        │ 자금 투입                       │
  ├─────────────────────────────────────┼────────┼─────────────────────────────────┤
  │ strategies/sells/ (매도 전략)       │ 0줄    │ 익절 실패 = 수익 기회 상실      │
  ├─────────────────────────────────────┼────────┼─────────────────────────────────┤
  │ services/account_trader.py (핵심    │ 0줄    │ 전체 매매 사이클 검증 없음      │
  │ 루프)                               │        │                                 │
  ├─────────────────────────────────────┼────────┼─────────────────────────────────┤
  │ api/admin.py (672줄)                │ 0줄    │ 관리자 기능 검증 없음           │
  └─────────────────────────────────────┴────────┴─────────────────────────────────┘

  실제 돈이 오가는 매수/매도 로직에 테스트가 전무한 건, 코드가 아무리 잘 작성되어
  있어도 변경 시 regression을 잡을 수 없다는 의미입니다.

  2. 장애 복구 시나리오

  - 서버 크래시 중 주문 상태: step() 도중 서버가 죽으면 거래소에는 주문이 올라갔는데
  DB에는 기록 안 된 상태가 될 수 있음. 현재 _sync_orders_and_fills가 다음 사이클에서
  복구하긴 하지만, 이 경로에 대한 테스트가 없음
  - Binance 장시간 장애: WS 재연결은 있지만, REST API도 동시에 죽으면 circuit breaker
  5회 후 전 계정 비활성화 → 수동 복구 필요
  - DB 장애 시 매매 중단 여부: DB 연결 실패 시 graceful degradation 없이 예외 발생

  3. 재무 정합성 검증 수단 부재

  - 거래소 실제 포지션과 DB 기록을 비교하는 reconciliation 기능이 없음
  - 장기 운영 시 fills 누락, 부분 체결 미반영 등으로 DB와 거래소 상태가 서서히 벌어질
  수 있음
  - 현재는 수동으로 Binance 대시보드와 비교해야 함

  4. 운영 인프라

  - CI/CD 파이프라인 없음: 수동 배포 → 실수 가능성
  - DB 백업 전략 미확인: Supabase 자동 백업에만 의존 중
  - 로그 수집/분석 체계 없음: 서버 로그가 stdout에만 출력, 장애 후 분석 어려움

  5. API 호출 & DB 최적화
  이 내용에 추가로 /analyze, /review, /code-review 중 적절한 스킬로 api나 db 등에서 발생하는 불필요한 호출이나 왕복, 중복 등의 최적화 테스트랑 개선 방안도 다시한번 확인해줘